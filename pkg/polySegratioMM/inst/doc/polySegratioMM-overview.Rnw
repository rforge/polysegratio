...
A Bayesian approach to marker dosage estimation was proposed by Baker
et al SUBMITTED whereby all markers are fitting a finite mixture
distribution.
...
\texttt{JAGS} must be installed and the executable must be in your
path. Note that no checking is carried out to asecrtain whether or not
\texttt{JAGS} is set up appropriately.
...
theoretical distribution is narrow than the observed data.
...
found in Baker, et al SUBMITTED
...
seleceted output is printed here the simple command
\texttt{print(mcmcHexRun)} whould producethe following output and
more.
...
convergence may result in mis--allocated marker doages. The diagnostic
statistics indicate that convergence was acheived.
<<>>=
...
When \texttt{plots} option of \texttt{runSegratioMM} is set to the
